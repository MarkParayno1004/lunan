...
describe('the useHandleRoomDisconnection hook', () => {
    let mockRoom = new EventEmitter();
    beforeEach(jest.clearAllMocks);
    it('should do nothing if the room emits a "disconnected" event with no error', () => {
        renderHook(() => useHandleRoomDisconnection(mockRoom, mockOnError, mockRemoveLocalAudioTrack, mockRemoveLocalVideoTrack, false, mockToggleScreenSharing));
        act(() => {
    ...
    it('should toggle screensharing when the "disconnected" event is emitted and isSharing is true', () => {
        renderHook(() => useHandleRoomDisconnection(mockRoom, mockOnError, mockRemoveLocalAudioTrack, mockRemoveLocalVideoTrack, true, mockToggleScreenSharing));
        act(() => {
            mockRoom.emit('disconnected', mockRoom, 'mockError');
        });
    ...
    it('should tear down old listeners when receiving a new room', () => {
        ...
        const { rerender } = renderHook(() => useHandleRoomDisconnection(mockRoom, mockOnError, mockRemoveLocalAudioTrack, mockRemoveLocalVideoTrack, false, mockToggleScreenSharing));
    ...
    it('should clean up listeners on unmount', () => {
        const { unmount } = renderHook(() => useHandleRoomDisconnection(mockRoom, mockOnError, mockRemoveLocalAudioTrack, mockRemoveLocalVideoTrack, false, mockToggleScreenSharing));
        unmount();
