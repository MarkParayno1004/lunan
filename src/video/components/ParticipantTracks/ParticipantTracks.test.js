...
describe('the ParticipantTracks component', () => {
    it('should render an array of publications', () => {
        const wrapper = shallow(React.createElement(ParticipantTracks, { participant: 'mockParticipant' }));
    ...
    it('should filter out any screen share publications', () => {
        ...
        expect(wrapper
            .find('Publication')
            .at(0)
            .prop('publication')).toEqual({ trackName: '', trackSid: 1, kind: 'video' });
    });
    describe('with enableScreenShare prop', () => {
        it('should filter out camera publications when a screen share publication is present', () => {
            ...
            const wrapper = shallow(React.createElement(ParticipantTracks, { participant: 'mockParticipant', enableScreenShare: true }));
            expect(wrapper.find('Publication').length).toBe(1);
            expect(wrapper
                .find('Publication')
                .at(0)
                .prop('publication')).toEqual({ trackName: 'screen', trackSid: 0, kind: 'video' });
        });
