...
describe('the Room component', () => {
    it('should render correctly when the chat window and background selection windows are closed', () => {
        const wrapper = shallow(React.createElement(Room, null));
...
describe('the useSetSpeakerViewOnScreenShare hook', () => {
    ...
    it('should not deactivate gallery view when there is no screen share participant', () => {
        renderHook(() => useSetSpeakerViewOnScreenShare(undefined, { localParticipant: {} }, mockSetIsGalleryViewActive, true));
    ...
    it('should deactivate gallery view when a remote participant shares their screen', () => {
        const { rerender } = renderHook(({ screenShareParticipant }) => useSetSpeakerViewOnScreenShare(screenShareParticipant, { localParticipant: {} }, mockSetIsGalleryViewActive, true), { initialProps: { screenShareParticipant: undefined } });
        expect(mockSetIsGalleryViewActive).not.toBeCalled();
        rerender({ screenShareParticipant: {} });
    ...
    it('should reactivate gallery view when screenshare ends if gallery view was active before another participant started screensharing', () => {
        ...
        expect(mockSetIsGalleryViewActive).toBeCalledWith(false);
        // screenshare ends
        rerender({ screenShareParticipant: undefined });
    ...
    it('should not activate gallery view when screenshare ends if speaker view was active before another participant started screensharing', () => {
        const { rerender } = renderHook(({ screenShareParticipant }) => useSetSpeakerViewOnScreenShare(screenShareParticipant, { localParticipant: {} }, mockSetIsGalleryViewActive, false), { initialProps: { screenShareParticipant: undefined } });
        expect(mockSetIsGalleryViewActive).not.toBeCalled();
        // screenshare starts
        rerender({ screenShareParticipant: {} });
        expect(mockSetIsGalleryViewActive).toBeCalledWith(false);
    ...
    it('should not activate speaker view when screenshare ends if it was active before screensharing, but the user switched to gallery view during the screenshare', () => {
        ...
        const { rerender } = renderHook(({ screenShareParticipant, isGalleryViewActive }) => useSetSpeakerViewOnScreenShare(screenShareParticipant, mockRoom, mockSetIsGalleryViewActive, isGalleryViewActive), { initialProps: { screenShareParticipant: undefined, isGalleryViewActive: false } });
        expect(mockSetIsGalleryViewActive).not.toBeCalled();
        // start screenshare
        rerender({ screenShareParticipant: mockScreenShareParticipant, isGalleryViewActive: false });
        expect(mockSetIsGalleryViewActive).toBeCalledWith(false);
        // enable gallery view
        rerender({ screenShareParticipant: mockScreenShareParticipant, isGalleryViewActive: true });
        // stop screenshare
        rerender({ screenShareParticipant: undefined, isGalleryViewActive: true });
    ...
    it('should not deactivate gallery view when the local participant shares their screen', () => {
        ...
        const { rerender } = renderHook(({ screenShareParticipant }) => useSetSpeakerViewOnScreenShare(screenShareParticipant, { localParticipant: mockLocalParticipant }, mockSetIsGalleryViewActive, true), { initialProps: { screenShareParticipant: undefined } });
        expect(mockSetIsGalleryViewActive).not.toBeCalled();
        rerender({ screenShareParticipant: mockLocalParticipant });
        expect(mockSetIsGalleryViewActive).not.toBeCalled();
    });
});
