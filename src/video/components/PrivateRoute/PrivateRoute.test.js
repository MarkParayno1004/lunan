...
jest.mock('../../state');
const MockComponent = () => React.createElement("h1", null, "test");
    ...
    describe('with auth enabled', () => {
        describe('when isAuthReady is true', () => {
            it('should redirect to /login when there is no user', () => {
                ...
                const wrapper = mount(React.createElement(MemoryRouter, { initialEntries: ['/'] },
                    React.createElement(PrivateRoute, { exact: true, path: "/" },
                        React.createElement(MockComponent, null))));
