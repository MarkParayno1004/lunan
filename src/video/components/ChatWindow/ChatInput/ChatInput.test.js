...
const mockHandleSendMessage = jest.fn(() => Promise.resolve());
describe('the ChatInput component', () => {
    ...
    it('should enable the send message button when user types a valid message', () => {
        const wrapper = shallow(React.createElement(ChatInput, { conversation: { sendMessage: mockHandleSendMessage }, isChatWindowOpen: true }));
        expect(wrapper
    ...
    it('should send a media message when a user selects a file', () => {
        ...
        var formData = mockHandleSendMessage.mock.calls[0][0];
    ...
    it('should add the "isTextareaFocused" class to the parent of TextareaAutosize when the focus event is fired, and remove it when the blur event is fired', () => {
        const wrapper = mount(React.createElement(ChatInput, { conversation: { sendMessage: mockHandleSendMessage }, isChatWindowOpen: true }));
        wrapper.find(TextareaAutosize).simulate('focus');
        expect(wrapper
            .find(TextareaAutosize)
            .parent()
            .prop('className')).toContain('isTextareaFocused');
        wrapper.find(TextareaAutosize).simulate('blur');
        expect(wrapper
            .find(TextareaAutosize)
            .parent()
            .prop('className')).not.toContain('isTextareaFocused');
    });
    ...
    it('should focus on the textarea element when the chat window is opened', () => {
        const wrapper = mount(React.createElement(ChatInput, { conversation: { sendMessage: mockHandleSendMessage }, isChatWindowOpen: false }));
        const textareaEl = wrapper
            .find('textarea')
            .at(0)
            .getDOMNode();
