...
const mockTrack = { attach: jest.fn(() => audioEl), detach: jest.fn(() => [audioEl]) };
jest.mock('../../state');
...
describe('the AudioTrack component', () => {
    ...
    it('should add an audio element to the DOM when the component mounts', () => {
        render(React.createElement(AudioTrack, { track: mockTrack }));
    ...
    it('should remove the audio element from the DOM when the component unmounts', () => {
        const { unmount } = render(React.createElement(AudioTrack, { track: mockTrack }));
        unmount();
